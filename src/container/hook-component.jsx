...
const Hooks = (props) => {
    ...
    useEffect(() => {
        document.title = `Title Change: ${props.state.globalcounter}`
        return () => {
            document.title = `React App`
        }
    });
    ...
    return(
                        ...
                        <div className="container">
                            {console.log(props)}
                                  <p>Jumlah Item : {props.state.globalcounter} </p>
                            ...
                            <button className="btn btn-primary" onClick={()=> props.dispatch({type: "RESET"})}>Reset</button>
                         </div>
